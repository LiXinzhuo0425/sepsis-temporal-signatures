#!/usr/bin/env python3
"""Build six manuscript manuscript figures with Python only."""
...
def figure1() -> None:
    ...
    # Final current patient-level audit (5 Sep 2026): the frozen input snapshot
    # retained historical unique-case-ID counts in the column labelled
    # "Baseline case patients" for GSE236713 and GSE54514.  Correct only the
    # figure-facing descriptive metadata here; paired-analysis inputs and all
    # inferential results are unchanged.
    observed_baseline_n = {
        "GSE236713": 124,
        "GSE57065": 28,
        "GSE95233": 51,
    ...
    cohorts["Unique case IDs"] = cohorts["Cohort"].map(unique_case_ids)
    cohorts["Baseline case patients"] = cohorts["Cohort"].map(observed_baseline_n)
    cohorts = cohorts.rename(columns={
        "Baseline case patients": "Baseline patients with observed sample",
        "Patients with ≥2 windows": "Patients with ≥2 recorded time categories",
    })
    ...
    boxes = [
        ...
        (0.355, 0.67, 0.29, 0.18, "299 baseline\npatients", BLUE),
